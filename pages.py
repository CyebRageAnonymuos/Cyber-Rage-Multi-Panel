...
.link-url-alt{color:#a78bfa;background:rgba(167,139,250,0.06);margin-top:6px;font-size:10px}
.link-url-alt:hover{background:rgba(167,139,250,0.14)}
...
.fi,.fs,.fta{padding:10px 14px;border-radius:10px;border:1px solid var(--card-b);background:rgba(0,0,0,.2);color:var(--t1);font-family:inherit;font-size:13px;outline:none;transition:.15s}
[data-theme="light"] .fi,[data-theme="light"] .fs,[data-theme="light"] .fta{background:rgba(0,0,0,.04)}
.fi::placeholder,.fta::placeholder{color:var(--t3)}
.port-presets{display:flex;gap:4px;flex-wrap:wrap}
.port-presets button{padding:3px 8px;font-size:10.5px;border-radius:6px;border:1px solid var(--card-b);background:rgba(0,240,255,.04);color:var(--accent);cursor:pointer;font-family:inherit;transition:.15s}
.port-presets button:hover{background:rgba(0,240,255,.14)}
.proto-warn{margin-top:6px;font-size:11px;line-height:1.5;color:#ffb347;background:rgba(255,179,71,.08);border:1px solid rgba(255,179,71,.25);border-radius:8px;padding:7px 10px;display:flex;gap:6px;align-items:flex-start}
.fi:focus,.fs:focus,.fta:focus{border-color:rgba(0,240,255,.35);box-shadow:0 0 0 3px rgba(0,240,255,.06)}
.fs option{background:var(--bg2)}
.fta{resize:vertical;min-height:50px;font-family:'JetBrains Mono',monospace;font-size:12px;line-height:1.6}
...
<div class="modal-bg" id="modal-create-link">
  <div class="modal">
    ...
    <div class="form-row">
      ...
      <div class="fg"><label>Protocol</label><select class="fs" id="cl-proto"><option value="vless-ws">VLESS + WebSocket</option><option value="vless-tcp">VLESS + TCP (fast)</option><option value="xhttp-packet-up">XHTTP (packet-up)</option><option value="xhttp-stream-up">XHTTP (stream-up)</option></select></div>
    </div>
    <div class="form-row">
      ...
      <div class="fg"><label>Port</label><input class="fi" id="cl-port" type="number" value="443" min="1" max="65535" oninput="markPortManual('cl')"><div class="port-presets"><button type="button" onclick="setPort('cl-port',443)">443</button><button type="button" onclick="setPort('cl-port',8443)">8443</button><button type="button" onclick="setPort('cl-port',2053)">2053</button><button type="button" onclick="setPort('cl-port',2083)">2083</button><button type="button" onclick="setPort('cl-port',2087)">2087</button><button type="button" onclick="setPort('cl-port',2096)">2096</button></div><div class="proto-warn" id="cl-warn" style="display:none"><i class="ph ph-warning"></i> TCP without TLS is blocked by Iranian ISPs. Use Cloudflare + Clean IP, or deploy with TLS_CERT/TLS_KEY.</div></div>
    </div>
    <div class="form-row">
      <div class="fg"><label>ALPN</label><input class="fi" id="cl-alpn" placeholder="http/1.1"></div>
      <div class="fg"><label>Fragment (bypass DPI)</label><input class="fi" id="cl-frag" placeholder="tcp,100,150"></div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
      <div class="fg" style="flex:100%"><label>Clean IP Repository</label><textarea class="fta" id="cl-ips" placeholder="e.g. 1.2.3.4&#10;5.6.7.8"></textarea></div>
    </div>
...
<div class="modal-bg" id="modal-edit-link">
  <div class="modal">
    ...
    <div class="form-row">
      ...
      <div class="fg"><label>Protocol</label><select class="fs" id="el-proto"><option value="vless-ws">VLESS + WebSocket</option><option value="vless-tcp">VLESS + TCP (fast)</option><option value="xhttp-packet-up">XHTTP (packet-up)</option><option value="xhttp-stream-up">XHTTP (stream-up)</option></select></div>
    </div>
    <div class="form-row">
      <div class="fg"><label>Fingerprint</label><select class="fs" id="el-fp"><option value="chrome">Chrome</option><option value="firefox">Firefox</option><option value="safari">Safari</option><option value="ios">iOS</option><option value="android">Android</option><option value="edge">Edge</option><option value="random">Random</option><option value="randomized">Randomized</option></select></div>
      <div class="fg"><label>Port</label><input class="fi" id="el-port" type="number" min="1" max="65535" oninput="markPortManual('el')"><div class="port-presets"><button type="button" onclick="setPort('el-port',443)">443</button><button type="button" onclick="setPort('el-port',8443)">8443</button><button type="button" onclick="setPort('el-port',2053)">2053</button><button type="button" onclick="setPort('el-port',2083)">2083</button><button type="button" onclick="setPort('el-port',2087)">2087</button><button type="button" onclick="setPort('el-port',2096)">2096</button></div><div class="proto-warn" id="el-warn" style="display:none"><i class="ph ph-warning"></i> TCP without TLS is blocked by Iranian ISPs. Use Cloudflare + Clean IP, or deploy with TLS_CERT/TLS_KEY.</div></div>
    </div>
    <div class="form-row">
      <div class="fg"><label>ALPN</label><input class="fi" id="el-alpn"></div>
      <div class="fg"><label>Fragment (bypass DPI)</label><input class="fi" id="el-frag" placeholder="tcp,100,150"></div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
      <div class="fg" style="flex:100%"><label>Note</label><input class="fi" id="el-note"></div>
    </div>
    <div class="form-row">
      <div class="fg" style="flex:100%"><label>Clean IP Repository</label><textarea class="fta" id="el-ips" placeholder="e.g. 1.2.3.4&#10;5.6.7.8"></textarea></div>
    </div>
    <div class="form-row" style="margin-top:8px">
...
let serverInfo={tcp_port:8443,tcp_tls:false};
async function loadInfo(){
  try{
    const r=await fetch('/api/info');
    serverInfo=await r.json();
    $('#cl-proto').addEventListener('change',()=>protoChanged('cl','new'));
    $('#el-proto').addEventListener('change',()=>protoChanged('el','edit'));
  }catch(e){}
}
function setPort(input,val){$('#'+input).value=val;$('#'+input).dataset.manual='1';}
function markPortManual(prefix){$('#'+prefix+'-port').dataset.manual='1';}
function protoChanged(prefix,mode){
  const p=$('#'+prefix+'-proto').value;
  const warn=$('#'+prefix+'-warn');
  const port=$('#'+prefix+'-port');
  if(p==='vless-tcp'){
    warn.style.display=serverInfo.tcp_tls?'none':'flex';
    if(!port.dataset.manual)port.value=serverInfo.tcp_port;
  }else{
    warn.style.display='none';
    if(!port.dataset.manual)port.value=443;
  }
}
function showCreateLink(){
  $('#cl-proto').value='vless-ws';
  $('#cl-port').value=443;
  delete $('#cl-port').dataset.manual;
  protoChanged('cl','new');
...
function renderLinks(){
  ...
  el.innerHTML=linksData.map(l=>{
    ...
    const domVariant=l.vless_link_domain?`<div class="link-url link-url-alt" onclick="copy(this)" title="Direct (Domain) config">${l.vless_link_domain}</div>`:'';
    return `<div class="list-item">
      <div class="list-icon">${status}</div>
      <div class="list-info">
        <div class="list-name">${l.label} ${badge}</div>
        <div class="list-sub">${l.protocol} &bull; ${l.fingerprint} &bull; Port ${l.port} &bull; ${l.connected_ips||0} IPs${l.clean_ips&&l.clean_ips.length?`<br><span style="color:var(--accent)"><i class="ph ph-check-circle"></i> ${l.clean_ips.length} Clean IP${l.clean_ips.length>1?'s':''}</span>`:''}</div>
        ${bar}
        ${expBlock}
        ${domVariant}
      </div>
...
async function createLink(){
  ...
  await api('/api/links',{method:'POST',body:JSON.stringify({
    ...
    port:parseInt($('#cl-port').value)||443,alpn:$('#cl-alpn').value,fragment:$('#cl-frag').value,
...
function editLink(uid){
  ...
  $('#el-frag').value=l.fragment||'';$('#el-ip').value=l.ip_limit||0;$('#el-note').value=l.note||'';
  ...
  $('#el-ips').value=(l.clean_ips||[]).join('\n');
  delete $('#el-port').dataset.manual;
  protoChanged('el','edit');
...
async function updateLink(){
  ...
  await api('/api/links/'+uid,{method:'PATCH',body:JSON.stringify({
    ...
    port:parseInt($('#el-port').value)||443,alpn:$('#el-alpn').value,fragment:$('#el-frag').value,
...
// Auto-refresh
loadInfo();loadStats();loadLinks();loadSubs();loadActivity();loadConnections();
